...
export function SubjectGroup({ title, color, studySets, subjects }: Props) {
  const [open, setOpen] = useState(true)
  if (studySets.length === 0) return null

  return (
    <section className="mb-8">
      <button className="flex items-center gap-2 mb-3" onClick={() => setOpen(o => !o)}>
        ...
        <div className="space-y-3">
          {studySets.map(s => (
            <StudySetCard key={s.id} studySet={s} subjects={subjects}
              onRename={() => {}} onDelete={() => {}} onRefresh={() => {}}
              onAssignSubject={() => {}} onAddDocument={() => {}} onEditPrompt={() => {}} />
          ))}
        </div>
      )}
    </section>
